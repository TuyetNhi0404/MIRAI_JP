...
const ExerciseList: React.FC<ExerciseListProps> = ({ exercises, onEdit, onDelete }) => {
    ...
    return (
      <Paper elevation={0} sx={{ p: 4, bgcolor: '#f9f9f9', borderRadius: 2, border: '1px dashed #ccc', textAlign: 'center' }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Chưa có bài tập nào. Nhấn 'Add Exercise' hoặc 'Import' để bắt đầu.
        </Typography>
      </Paper>
    );
  }
  ...
  const getTypeChip = (type: string) => {
    switch (type) {
      case 'quiz':
        return <Chip label="Quiz" size="small" sx={{ bgcolor: '#ffebee', color: '#B90000', fontWeight: 'bold' }} />;
      case 'fill_blank':
        return <Chip label="Fill Blank" size="small" color="warning" sx={{ fontWeight: 'bold' }} />;
      case 'dictation':
        return <Chip label="Dictation" size="small" color="info" sx={{ fontWeight: 'bold' }} />;
      default:
        return null;
    }
  };
